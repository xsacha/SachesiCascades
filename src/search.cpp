...
void Search::updateDetailRequest(int country, int carrier, int device)
{
    QString up = "upgrade";
    ...
    if (isNewer) {
        ...
        if (ver == "") {
            ...
            _updateMessage = QString(tr("Update %1 (%2 | %3) is available for %4!"))
                        .arg(ver)
                        .arg(os != "" ? QString(tr("OS: %1")).arg(os) : "")
                        .arg(radio != "" ? QString(tr("Radio: %1")).arg(radio) : "")
                        .arg(variant);
...
void Search::setScanning(const int &scanning) {
    if (_scanning != 0 && scanning == 0 && _updateMessage == "") {
